...
namespace Detector
{
    ...
    std::vector<std::shared_ptr<Object>> CueDetector::getObjects()
    {
        ...
        for(auto& processor : processors)
        {
            ...
            {
                data = std::make_shared<Balls>(balls.pos, balls.radius);
            }
    ...
    void CueDetector::setBalls(std::vector<cv::Point> balls, double radius)
    {
        this->balls.pos = balls;
        this->balls.radius = radius;
    }
}//namespace Detector
